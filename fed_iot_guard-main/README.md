SRF-Mal extension (Jessmon & Alwan, 2026)
This project extends the original fed_iot_guard codebase with scalable robust federated learning experiments for IoT malware detection.

Main experiment setting used in the report:
- 80 clients
- 32 malicious clients (40%)
- 30 communiation rounds
- 10 local epochs
- Aggregation methods: FedAvg, Median, Trimmed Mean, Krum, Bulyan
- Attacks: none, label_flip, gradient_noise, sign_flip

Dataset:
- N-BaIoT subset using 3 device types
- Set DATA_PATH in src/main.py to your local dataset folder before training

Example command:
python src/main.py --srf --clients 80 --mal 40 --rounds 30 --strategy bulyan --attack sign_flip
