...
# ── Device mapping ─────────────────────────────────────────────────────────────
# Your files use number prefixes (1, 2, 3...) instead of device name folders.
# We map each number to the corresponding device name from Rey et al.
...
# Devices that can be infected by Mirai (same as Rey et al.)
...
# ── UPDATED: your flat folder path ────────────────────────────────────────────
# Change this to your actual folder path
data_path = r'C:\Users\Mustafa Shalan\Downloads\archive' + '\\'

# ── UPDATED: file path builders for your flat naming format ───────────────────
# Your files are named like: 1.benign.csv, 1.mirai.ack.csv, 1.gafgyt.combo.csv
# Device index starts at 1 (not 0)

def _device_num(device: str) -> int:
    """Convert device name to its 1-based number prefix in your files."""
    return all_devices.index(device) + 1

def _benign_path(device: str) -> str:
    return data_path + f'{_device_num(device)}.benign.csv'

def _mirai_path(device: str, attack: str) -> str:
    return data_path + f'{_device_num(device)}.mirai.{attack}.csv'

def _gafgyt_path(device: str, attack: str) -> str:
    return data_path + f'{_device_num(device)}.gafgyt.{attack}.csv'

# Build path dictionaries matching Rey et al.'s original structure
benign_paths = {device: _benign_path(device) for device in all_devices}

mirai_paths = [{device: _mirai_path(device, attack) for device in mirai_devices}
               for attack in mirai_attacks]

gafgyt_paths = [{device: _gafgyt_path(device, attack) for device in all_devices}
                for attack in gafgyt_attacks]

# ── Everything below is UNCHANGED from Rey et al. ─────────────────────────────
