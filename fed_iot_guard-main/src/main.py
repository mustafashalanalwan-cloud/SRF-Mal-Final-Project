...
# -- Added by SRF-Mal (Jessmon & Alwan, 2026) -----------------------------------------
from srf_experiments import run_srf
# Change this path to your local N-BaIoT dataset folder
...
if __name__ == "__main__":
    ...
    # -- Original Rey et al. arguments (unchanged) ---------------------------------

    parser.add_argument('setup', nargs='?', help='centralized or decentralized')
    parser.add_argument('experiment', nargs='?', help='Experiment to run (classifier or autoencoder)')
    ...
    parser.add_argument('--srf', action='store_true',
                        help='Run SRF-Mal experiment (Krum, Bulyan, + attacks)')
    parser.add_argument('-all',  action='store_true',
                        help='[SRF] Sweep all strategies x attacks')
    parser.add_argument('--quick', action='store_true',
                        help='[SRF] Quick test: 2 strategies, 2 attacks, 10 rounds')
    parser.add_argument('--strategy', type=str, default='fedavg',
                        choices=['fedavg', 'median', 'trimmed_mean', 'krum', 'bulyan'])
    parser.add_argument('--attack', type=str, default='none',
                        choices=['none', 'label_flip','gradient_noise','sign_flip'])
    parser.add_argument('--rounds', type=int, default=30)
    parser.add_argument('--clients', type=int, default=10)
    parser.add_argument("--mal", type=int, default=40, help='Malicious percentage:20 or 40')
    #-------------------------------------------------------------------------------------
...
# -- Route to SRF-Mal or Rey et al. original ----------------------------------------------
    if args.srf:
      run_srf(
        data_path            = DATA_PATH,
        ...
        malicious_pct        = args.mal,
        results_dir          = 'results',
    )
    else:
    # Original Rey et al. experiment
